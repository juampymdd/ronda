...
export function OrdersView() {
    ...
    const [staleResult, setStaleResult] = useState<{ updated: number; error?: string } | null>(null);

    const queryClient = useQueryClient();

    const { data, isLoading } = useQuery({
        queryKey: ["orders", currentPage, pageSize, filterStatus, dateFrom, dateTo],
        queryFn: () => fetchOrders(currentPage, pageSize, filterStatus, dateFrom, dateTo),
    ...
    const handleCloseDia = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setStaleResult({ updated: 0, error: data.error ?? "Error desconocido" });
            }
        } catch (err) {
            console.error("Error cerrando pedidos del día:", err);
            setStaleResult({ updated: 0, error: "No se pudo conectar con el servidor" });
        } finally {
            ...
            <div className="glass-card p-4">
                <div className="flex items-center gap-4 flex-wrap">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-white">Filtrar por fecha:</span>
                    </div>
                ...
                <div className="flex items-center justify-between glass-card p-4 flex-wrap gap-4">
                    <div className="flex items-center gap-4">
                        <div className="text-sm text-slate-400">
                            ...
                            de{" "}
                            <span className="font-bold text-white">{pagination.total}</span> pedidos
                        </div>
